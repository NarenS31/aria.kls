...
_Generated 2026-07-24 17:16Z from the result JSONs; updated 2026-07-29 after the generator-agnostic heuristic repair and research-readiness audit. Every row's tier is applied strictly; nothing here is aspirational._
...
**Tier distribution:** A=0, B=2, C=1, D=6, E=4
...
| Conversational intent routing on varied student phrases | trained Eedi-derived weak-label model plus 72-case developer challenge | D | 72 challenge cases | The challenge was used during development and is not independent; Eedi intent labels are weak supervision, not human ground truth. |
| Observable reasoning-move extraction | transparent multi-label pattern baseline with exact spans | E | — | Implementation exists, but independent human annotation and a student/session-disjoint test are pending. |
...
The strongest ARIA claims are Tier B/C. **No claim is Tier A** until a real
student-language corpus with independent human labels is evaluated on a locked,
student/session-disjoint test set. Anything still at Tier D is validated only
against synthetic or development labels; Tier E is asserted from design
